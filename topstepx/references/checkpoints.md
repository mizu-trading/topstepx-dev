...
**TSX framework context:** This reference is used by tsx-executor and tsx-planner agents when creating and executing trading bot plans. The tsx-verifier agent uses these checkpoint patterns to validate human-verify gates during execution.
...
<trading_specific_checkpoints>

## Trading-Specific Checkpoints

These checkpoint examples are specific to TopStepX trading bot development. They demonstrate common verification gates for tsx-trading-bot workflows. The tsx-executor agent encounters these checkpoints when building trading integrations. The tsx-planner agent should include these checkpoint types in plans that involve API connections, WebSocket streaming, order placement, or risk guardrails. See also: verification-patterns.md for how to programmatically verify trading artifacts.

### 1. API Connection Checkpoint

Verify the bot authenticates with TopStepX and receives a valid JWT.
...
**Why this checkpoint exists:** TopStepX requires a valid JWT for all API and WebSocket operations. The token has a 24-hour TTL. If authentication fails or the refresh timer is not set, the bot will silently stop working after 24 hours. This checkpoint catches both immediate auth failures and missing refresh logic.

**What Claude automates before this checkpoint:**
- Writes the authentication module with TopStepX `POST /api/Auth/loginKey` call
- Implements proactive token refresh timer (see safety-patterns.md SAF-02)
- Starts the bot process

### 2. WebSocket Streaming Checkpoint

Verify real-time market data flows through the SignalR WebSocket connection.
...
**Why this checkpoint exists:** WebSocket connections to `rtc.topstepx.com` require proper SignalR handshake, JWT token injection, and automatic reconnection. Quote data must flow continuously for the bot to make trading decisions. If the connection drops without reconnecting, the bot may miss signals or trade on stale data.

**What Claude automates before this checkpoint:**
- Implements SignalR HubConnectionBuilder with `.withAutomaticReconnect()`
- Injects JWT token via `accessTokenFactory`
- Subscribes to `GotQuote` events for the target instrument
- Re-establishes subscriptions on reconnect

### 3. Order Placement Checkpoint

Verify the bot can place and manage orders on the exchange.
...
**Why this checkpoint exists:** Order placement is the most consequential operation a trading bot performs. Incorrect orders can result in financial loss. This checkpoint verifies: (a) orders use named enum constants (OrderSide.Bid, not 0), (b) bracket orders attach take-profit and stop-loss, (c) position sizing respects limits, (d) cancellation works. See safety-patterns.md SAF-01 for enum requirements.

**What Claude automates before this checkpoint:**
- Implements order placement using named enum constants
- Implements bracket order attachment (take-profit + stop-loss)
- Implements order cancellation via `POST /api/Order/cancel`
- Runs the bot in evaluation/simulation account mode
- Logs all order operations to console

### 4. Risk Guardrails Checkpoint

Verify safety limits prevent excessive exposure.
...
**Why this checkpoint exists:** Risk guardrails are the last line of defense against runaway trading bots. Without them, a bot could open unlimited positions, exceed daily loss limits, or trade through account drawdown thresholds. This checkpoint is essential before any live or funded account deployment.

**What Claude automates before this checkpoint:**
- Implements max position size enforcement
- Implements daily loss tracking
- Runs grep check: `grep -r "side: [0-9]" src/` returns 0 results (no bare enum integers)
- Runs grep check: `grep -r "type: [0-9]" src/` returns 0 results
- Documents risk parameters in bot configuration

### Trading Checkpoint Placement Guide

In a typical tsx-trading-bot plan (created by tsx-planner, executed by tsx-executor), checkpoints appear in this order:

1. **API Connection** -- After auth module is built, before any API-dependent work
2. **WebSocket Streaming** -- After real-time connection is built, before order logic depends on quotes
3. **Order Placement** -- After order module is built, before strategy integration
4. **Risk Guardrails** -- After all trading logic is built, before declaring the bot ready for deployment

Each checkpoint builds on the previous one. Do NOT skip to Order Placement without verifying WebSocket works, because order timing often depends on real-time quote data.

**TSX command integration:** When using `/tsx:execute-phase` or `/tsx:execute-plan`, trading checkpoints follow the same auto-mode rules as standard checkpoints. In auto-mode, tsx-executor will auto-approve trading human-verify checkpoints, which is appropriate for CI/pipeline runs but should be disabled for first-time bot deployment to a funded account.
...
- API Connection checkpoint verifies JWT auth and refresh timer
- WebSocket Streaming checkpoint verifies SignalR connection and auto-reconnect
- Order Placement checkpoint verifies enum usage, bracket orders, and cancellation
- Risk Guardrails checkpoint verifies position limits and daily loss tracking
- See verification-patterns.md for programmatic trading verification patterns
</summary>
