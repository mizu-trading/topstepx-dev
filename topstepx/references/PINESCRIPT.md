...
> **CRITICAL:** Before converting any PineScript strategy, audit it for repainting. A strategy that repaints appears profitable in backtesting but trades on signals that would NOT have existed in real-time. See safety-patterns.md SAF-04 for the complete audit checklist with code examples.

### The Four Repainting Checks

**Check 1: `request.security()` without `[1]` offset + `lookahead_on`**

```pine
// REPAINTS -- uses future data:
htfClose = request.security(syminfo.tickerid, "D", close)

// SAFE -- confirmed previous bar only:
htfClose = request.security(syminfo.tickerid, "D", close[1], lookahead=barmerge.lookahead_on)
```

The `[1]` offset and `lookahead_on` are INTERDEPENDENT. You need BOTH. Using `lookahead_on` without `[1]` still repaints. Using `[1]` without `lookahead_on` gets stale data.

**Check 2: Conditions on unconfirmed bars (need `barstate.isconfirmed`)**
...
// SAFE -- only acts on confirmed bar close:
if ta.crossover(close, ma) and barstate.isconfirmed
    strategy.entry("Long", strategy.long)
```

**Check 3: Realtime-only logic (`barstate.isrealtime`)**

```pine
// REPAINTS -- different behavior on historical vs realtime:
...
**Check 4: Fluid values in conditions (high/low/close without `[1]`)**
...
**Converted bots should use bar-close signals (`barstate.isconfirmed`) unless the user explicitly requests tick-based execution.** When in doubt, require `barstate.isconfirmed` for all entry/exit conditions. This trades slightly later but eliminates repainting risk entirely.

---

## TA Function Conversion Mapping

When converting `ta.*` functions to TopStepX bot code, agents should use a TA library rather than reimplementing from scratch:

| PineScript `ta.*` Function | JS (`trading-signals`) | Python (`pandas-ta`) |
|----------------------------|------------------------|----------------------|
...
| `ta.stoch(close, high, low, len)` | `new Stochastic(len)` | `ta.stoch(high, low, close, k=len)` |
| `ta.crossover(a, b)` | `CrossOver` or manual `a[i-1] < b[i-1] && a[i] > b[i]` | Manual comparison |
| `ta.crossunder(a, b)` | `CrossUnder` or manual `a[i-1] > b[i-1] && a[i] < b[i]` | Manual comparison |

### Alert Condition Conversion

| PineScript | TopStepX Equivalent |
|------------|---------------------|
| `alertcondition(condition, ...)` | Event handler / callback that triggers order logic when condition is true |
| `alert("message")` | Logging + optional webhook/notification in bot code |
...
The following safety pattern in `safety-patterns.md` is directly related to PineScript conversion:

| Safety Pattern | Topic | Relevance |
|----------------|-------|-----------|
| **SAF-04** | PineScript Repainting Audit | Complete 4-point checklist with REPAINTS vs SAFE code examples for every repainting pattern |

All PineScript conversion workflows MUST run the SAF-04 repainting audit before generating bot code. See `safety-patterns.md` for the complete checklist and code patterns.
