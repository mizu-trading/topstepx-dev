<purpose>
Create all phases necessary to close gaps identified by `/tsx:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/tsx:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/tsx:audit-milestone` first.
```
...
PHASES=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" phases list)
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/tsx:execute-phase {N}` — if plans already exist
...
`/tsx:audit-milestone` — re-audit to verify gaps closed
`/tsx:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/tsx:plan-phase` next
</success_criteria>
