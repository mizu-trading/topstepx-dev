...
```bash
AUDITOR_MODEL=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" resolve-model tsx-nyquist-auditor --raw)
NYQUIST_CFG=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" config get workflow.nyquist_validation --raw)
```

If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /tsx:settings."

Display banner: `TSX > VALIDATE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /tsx:execute-phase {N} first."
...
## 5. Spawn tsx-nyquist-auditor

```
Task(
  prompt="Read $HOME/.claude/agents/tsx-nyquist-auditor.md for instructions.\n\n" +
    "<files_to_read>{PLAN, SUMMARY, impl files, VALIDATION.md}</files_to_read>" +
    "<gaps>{gap list}</gaps>" +
    "<test_infrastructure>{framework, config, commands}</test_infrastructure>" +
    "<constraints>Never modify impl files. Max 3 debug iterations. Escalate impl bugs.</constraints>",
  subagent_type="tsx-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `$HOME/.claude/topstepx/templates/VALIDATION.md`
...
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit-docs "docs(phase-${PHASE}): add/update validation strategy"
```
...
**Compliant:**
```
TSX > PHASE {N} IS NYQUIST-COMPLIANT
All requirements have automated verification.
▶ Next: /tsx:audit-milestone
```

**Partial:**
```
TSX > PHASE {N} VALIDATED (PARTIAL)
{M} automated, {K} manual-only.
▶ Retry: /tsx:validate-phase {N}
```
