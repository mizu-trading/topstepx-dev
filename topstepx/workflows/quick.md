<purpose>
Execute small, ad-hoc tasks with TSX guarantees (atomic commits, STATE.md tracking). Quick mode spawns tsx-planner (quick mode) + tsx-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```bash
INIT=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" init quick "$DESCRIPTION")
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/tsx:new-project` first.
...
Spawn tsx-executor with plan reference:
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
...
TSX > QUICK TASK COMPLETE (FULL MODE)
