...
<required_reading>
@$HOME/.claude/topstepx/references/verification-patterns.md
@$HOME/.claude/topstepx/templates/verification-report.md
</required_reading>
...
Use tsx-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use tsx-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" verify artifacts "$plan")
...
Use tsx-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" verify key-links "$plan")
...
**Always needs human:** Visual appearance, user flow completion, real-time behavior (WebSocket/SSE/SignalR), external service integration, performance feel, error message clarity.
...
See $HOME/.claude/topstepx/templates/verification-report.md for complete template.
</step>
