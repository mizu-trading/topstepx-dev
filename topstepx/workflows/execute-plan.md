...
@$HOME/.claude/topstepx/references/git-integration.md
</required_reading>
...
# config settings can be fetched via tsx-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="tsx-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn tsx-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" phases list --type summaries --raw
...
See `$HOME/.claude/topstepx/references/tdd.md` for structure.
</tdd_plan_execution>
...
See $HOME/.claude/topstepx/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `$HOME/.claude/topstepx/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `$HOME/.claude/topstepx/templates/summary.md`.
...
Update STATE.md using tsx-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" state record-metric \
...
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using tsx-tools:
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/tsx:execute-phase {phase}` + `/tsx:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/tsx:plan-phase {Z+1}` + `/tsx:verify-work {Z}` + `/tsx:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/tsx:complete-milestone` + `/tsx:verify-work` + `/tsx:add-phase` |
