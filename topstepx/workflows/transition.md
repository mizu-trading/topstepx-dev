...
**Delegate ROADMAP.md and STATE.md updates to tsx-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `tsx-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `tsx-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/tsx:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/tsx:discuss-phase [X+1] --auto")
...
`/tsx:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/tsx:plan-phase [X+1]` — skip discussion, plan directly
- `/tsx:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/tsx:discuss-phase [X+1]` — revisit context
- `/tsx:research-phase [X+1]` — investigate unknowns
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" config-set workflow._auto_chain_active false
```
...
Exit skill and invoke SlashCommand("/tsx:complete-milestone {version}")
