...
```bash
INIT=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" resolve-model tsx-integration-checker --raw)
```
...
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="tsx-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/tsx:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/tsx:validate-phase {N}` |

Phases needing validation: run `/tsx:validate-phase {N}` for each flagged phase.
...
- /tsx:complete-milestone {version} — proceed anyway (accept tech debt)
