...
Usage:
  /tsx:new-project --auto @your-idea.md
  /tsx:new-project --auto [paste or write your idea here]
...
**If `project_exists` is true:** Error — project already initialized. Use `/tsx:progress`.
...
- options:
  - "Map codebase first" — Run /tsx:map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**
```
Run `/tsx:map-codebase` first, then return to `/tsx:new-project`
```
Exit command.
...
```bash
mkdir -p .planning
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "chore: add project config" --files .planning/config.json
```
...
**Check for global defaults** at `~/.tsx/defaults.json`. If the file exists, offer to use saved defaults:

```
AskUserQuestion([
  {
    question: "Use your saved default settings? (from ~/.tsx/defaults.json)",
    header: "Defaults",
    multiSelect: false,
    options: [
...
If "Yes": read `~/.tsx/defaults.json`, use those values for config.json, and skip directly to **Commit config.json** below.

If "No" or `~/.tsx/defaults.json` doesn't exist: proceed with the questions below.
...
**Note:** Run `/tsx:settings` anytime to update these preferences.
...
Spawn 4 parallel tsx-project-researcher agents with path references:
...
Use template: $HOME/.claude/topstepx/templates/research-project/STACK.md
</output>
", subagent_type="tsx-project-researcher", model="{researcher_model}", description="Stack research")
...
Use template: $HOME/.claude/topstepx/templates/research-project/FEATURES.md
</output>
", subagent_type="tsx-project-researcher", model="{researcher_model}", description="Features research")
...
Use template: $HOME/.claude/topstepx/templates/research-project/ARCHITECTURE.md
</output>
", subagent_type="tsx-project-researcher", model="{researcher_model}", description="Architecture research")
...
Use template: $HOME/.claude/topstepx/templates/research-project/PITFALLS.md
</output>
", subagent_type="tsx-project-researcher", model="{researcher_model}", description="Pitfalls research")
```
...
Use template: $HOME/.claude/topstepx/templates/research-project/SUMMARY.md
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 TSX ► CREATING ROADMAP
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning roadmapper...
```

Spawn tsx-roadmapper agent with path references:
...
", subagent_type="tsx-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="tsx-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
Exit skill and invoke SlashCommand("/tsx:discuss-phase 1 --auto")
...
- [ ] User knows next step is `/tsx:discuss-phase 1`
