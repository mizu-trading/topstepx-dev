...
1. **tsx-researcher** — Reads CONTEXT.md to understand domain priorities for general research
   - "Trading bot using bar-close signals" → researcher focuses on signal confirmation patterns
   - "REST API preferred over WebSocket for orders" → researcher investigates REST client patterns

2. **tsx-phase-researcher** — Reads CONTEXT.md to know WHAT to research for this phase
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

3. **tsx-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/tsx:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" init phase-op "${PHASE}")
...
Use /tsx:progress to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /tsx:plan-phase {X} to replan
...
Extract key terms from the phase goal (e.g., "feed" → "post", "card", "list"; "auth" → "login", "session", "token"; "order" → "bracket", "position", "stop-loss"; "data" → "SignalR", "subscription", "WebSocket").
...
**Trading code pattern awareness:** When scanning a trading project codebase, additionally look for:
- Order placement patterns (bracket orders, market/limit/stop order builders, position sizing)
- Position management (open/close/modify, P&L tracking, max position limits)
- API integration patterns (REST client setup, authentication/JWT handling, rate limiting)
- WebSocket/SignalR subscription setup (hub connections, reconnection handlers, message dispatching)
- Strategy signal patterns (entry/exit logic, indicator calculations, bar-close confirmation)

```bash
# Trading-specific code patterns
grep -rl "placeOrder\|bracket\|stopLoss\|takeProfit" src/ --include="*.ts" --include="*.js" 2>/dev/null | head -5
grep -rl "SignalR\|signalr\|WebSocket\|subscribe" src/ --include="*.ts" --include="*.js" 2>/dev/null | head -5
grep -rl "position\|getPositions\|closePosition" src/ --include="*.ts" --include="*.js" 2>/dev/null | head -5
```
...
**Trading project phase awareness:** When analyzing trading-related phases, consider these additional domain boundaries:
- **API integration phases** — Auth flow, endpoint selection, error handling strategy, rate limit approach
- **Strategy logic phases** — Signal confirmation model (bar-close vs tick), indicator parameters, entry/exit rules
- **Risk management phases** — Position sizing, max loss limits, bracket defaults (stop-loss/take-profit), daily loss caps
- **Data streaming phases** — Which SignalR hubs, reconnection policy, data buffering, heartbeat handling
...
**Also available:**
- `/tsx:plan-phase ${PHASE} --skip-research` — plan without research
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
2. **Sync chain flag with intent** — if user invoked manually (no `--auto`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
     node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
3. Read both the chain flag and user preference:
   ```bash
   AUTO_CHAIN=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```
...
```bash
node "$HOME/.claude/topstepx/bin/tsx-tools.cjs" config-set workflow._auto_chain_active true
```
...
```
Skill(skill="tsx:plan-phase", args="${PHASE} --auto")
```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   TSX ► PHASE ${PHASE} COMPLETE
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished: discuss → plan → execute

  Next: /tsx:discuss-phase ${NEXT_PHASE} --auto
  <sub>/clear first → fresh context window</sub>
  ```
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /tsx:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /tsx:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /tsx:plan-phase ${PHASE} --gaps
  ```
...
- Trading code patterns scanned (order placement, position management, API integration, WebSocket subscriptions)
- Gray areas identified through intelligent analysis with code and prior decision annotations
- Trading-specific gray areas generated for relevant phases (strategy parameters, risk management, execution model, data subscriptions)
