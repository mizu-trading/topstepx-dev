...
> **Note:** Planning methodology is in `agents/tsx-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @$HOME/.claude/topstepx/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/tsx:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `$HOME/.claude/topstepx/references/tdd.md` for TDD plan structure.
...
**Trading plan example (order placement with bracket orders):**
...
wave: 1
depends_on: []
files_modified: [src/orders/orderManager.ts, src/orders/bracketOrder.ts]
autonomous: true
requirements: [STRAT-02]
---

<objective>
Implement order placement with mandatory bracket orders (stop-loss + take-profit).

Purpose: Every order entry must have bracket protection per SAF-01 risk guardrails.
Output: OrderManager with bracket order creation, validation, and TopStepX API integration.
</objective>
...
<tasks>
<task type="auto">
  <name>Task 1: Create OrderManager with bracket order defaults</name>
  <files>src/orders/orderManager.ts</files>
  <action>
    Create OrderManager class that:
    1. Accepts order parameters (side, size, contractId, accountId)
    2. Enforces bracket orders on every entry (stop-loss + take-profit)
    3. Uses enum constants from SAF-01 (never bare integers for OrderSide, OrderType)
    4. Validates position size against configurable max (default: 5 contracts)
    5. Calls TopStepX placeOrder endpoint with bracket order payload
  </action>
  <verify>Unit tests pass for order creation with bracket validation</verify>
  <done>OrderManager creates orders with mandatory brackets, rejects orders without stop-loss</done>
</task>

<task type="auto">
  <name>Task 2: Add order status tracking and rejection handling</name>
  <files>src/orders/bracketOrder.ts</files>
  <action>
    Implement bracket order status tracking:
    1. Monitor order status via WebSocket orderStatus subscription
    2. Handle OrderStatus.Rejected with single retry after 1s backoff
    3. Log all order events with timestamps
    4. Alert on rejection after retry exhaustion
  </action>
  <verify>Simulated rejection triggers retry and logging</verify>
  <done>Order rejections are retried once and logged with full context</done>
</task>
</tasks>

<verification>
- [ ] All orders include stop-loss and take-profit brackets
- [ ] No bare integer enum values in order payloads
- [ ] Position size validation rejects oversized orders
- [ ] Order rejection retry logic works correctly
</verification>

<success_criteria>
- Orders always include bracket protection
- Enum constants used throughout (SAF-01)
- Position sizing enforced
</success_criteria>

<output>
After completion, create `.planning/phases/02-strategy/02-02-SUMMARY.md`
</output>
```
...
See `$HOME/.claude/topstepx/templates/user-setup.md` for full schema and examples
...
Task completion does not equal goal achievement. A task "create chat component" can complete by creating a placeholder. The `must_haves` field captures what must actually work, enabling verification to catch gaps before they compound.
...
See `$HOME/.claude/topstepx/workflows/verify-phase.md` for verification logic.
